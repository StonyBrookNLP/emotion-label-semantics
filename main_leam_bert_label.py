...
from pybert.train.trainer_label_attn1 import Trainer
from torch.utils.data import DataLoader
from pybert.io.bert_processor_label_attention2 import BertProcessor
from pybert.common.tools import init_logger, logger
from pybert.common.tools import seed_everything
from pybert.configs.basic_config_leam2 import config
from pybert.model.nn.bert_for_multi_label_attention2_no_pretrain import BertForMultiLable
...
def run_test(args):
    ...
    from pybert.test.predictor_label_attention1 import Predictor
    import os
    import pickle
    ...
    test_examples = processor.create_examples(lines=test_data,
                                              example_type='test',
                                              cached_examples_file=config[
                                                                       'data_cache'] / f"cached_label_test_examples_{args.arch}")
    test_features = processor.create_features(examples=test_examples,
                                              max_seq_len=args.eval_max_seq_len,
                                              cached_features_file=config[
                                                                       'data_cache'] / "cached_label_test_features_{}_{}".format(
                                                  args.eval_max_seq_len, args.arch
                                              ))
    ...
    # pickle.dump(label_graph, open(os.path.join(config["checkpoint_dir"], "learned_label_corr.p"), "wb"))
